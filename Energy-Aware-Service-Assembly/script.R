...
overall <- read.table("exp_assembly_overall_energy_template_1664354010488.txt", 
                      header = F,
                      sep = " ",
                      col.names= c("overallx", "overally")
)

random <- read.table("exp_assembly_random_1664287138464.txt", 
                     header = F,
                     sep = " ",
                     col.names= c("randomx", "randomy")
)

res <- read.table("exp_assembly_residual_life_template_1664357104105.txt", 
                  header = F,
                  sep = " ",
                  col.names= c("resx","resy")
)

local <- read.table("exp_assembly_local_energy_template_1664291277547.txt", 
                    header = F,
                    sep = " ",
                    col.names= c("locx","locy")
)

energyAware <- read.table("exp_assembly_energyAware_1664286893560.txt", 
                          header = F,
                          sep = " ",
                          col.names= c("enx","eny")
)
